...
struct ReprojectionError
{
 ...
 if (ceres::abs(projected[2]) < T(1e-8))
      return false; // invalid, Ceres ignora questo residual

 // Normalize obtaining non homogeneous point
...
bool BasicSfM::incrementalReconstruction( int seed_pair_idx0, int seed_pair_idx1 )
{
  ...
  //Extract E and H and count the inliers of each geometric model
  cv::Mat E = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, prob, threshold, iters, inlier_mask_E  );
  cv::Mat H = cv::findHomography(points0, points1, cv::RANSAC, threshold, inlier_mask_H );

  cont_E = cv::countNonZero(inlier_mask_E);
  cont_H = cv::countNonZero(inlier_mask_H);
  ...
  //Try to recover R and t
  cv::Mat temp_R, temp_trans;
  cv::recoverPose(E, points0, points1, intrinsics_matrix, temp_R, temp_trans, inlier_mask_E);

  
  //Store values of t and check if the movement is mainly done sideways
  double translation_x = temp_trans.at<double>(0);
  double translation_y = temp_trans.at<double>(1);
  double translation_z = temp_trans.at<double>(2);

  double norm = cv::norm(temp_trans);
  double normalized_x = fabs(translation_x) / norm;
  double normalized_y = fabs(translation_y) / norm;
  double normalized_z = fabs(translation_z) / norm;


  if (normalized_x >= 0.5 || normalized_y >= 0.5) 
  //if (normalized_z <= 0.90)
  {
    temp_R.convertTo(init_r_mat, CV_64F);
    temp_trans.convertTo(init_t_vec, CV_64F);
  }
  ...
  for(int iter = 1; iter < num_cam_poses_ - 1; iter++ )
  {
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation_[cam_idx] )
        {
              ...
              cam_observation_[new_cam_pose_idx].find(pt_idx ) != cam_observation_[new_cam_pose_idx].end() )
          {
            ...
            //Store camera parameters for camera 0 and 1
            cv::Mat cam0_axis_angle_repr = (cv::Mat_<double>(3,1) << cam0_data[0], cam0_data[1], cam0_data[2]);
            cv::Mat cam0_translation_vector = (cv::Mat_<double>(3,1) << cam0_data[3], cam0_data[4], cam0_data[5]);

            cv::Mat cam1_axis_angle_repr = (cv::Mat_<double>(3,1) << cam1_data[0], cam1_data[1], cam1_data[2]);
            cv::Mat cam1_translation_vector = (cv::Mat_<double>(3,1) << cam1_data[3], cam1_data[4], cam1_data[5]);

            //Obtain rotation matrix from axis angle representation
            cv::Mat cam0_rotation_matrix, cam1_rotation_matrix;
            cv::Rodrigues(cam0_axis_angle_repr, cam0_rotation_matrix);
            cv::Rodrigues(cam1_axis_angle_repr, cam1_rotation_matrix);

            //Obtain projection matrix concatenating rotation and translaction
            cv::Mat cam0_projection_matrix, cam1_projection_matrix;
            cv::hconcat(cam0_rotation_matrix, cam0_translation_vector, cam0_projection_matrix);
            cv::hconcat(cam1_rotation_matrix, cam1_translation_vector, cam1_projection_matrix); 

            std::vector<cv::Point2d> point0(1), point1(1);

            //Obtain point observation coordinates
            point1[0] = cv::Point2d( observations_[2 * cam_observation_[cam_idx][pt_idx]],
                                     observations_[2 * cam_observation_[cam_idx][pt_idx] + 1 ]);

            point0[0] = cv::Point2d( observations_[2 * cam_observation_[new_cam_pose_idx][pt_idx]],
                                     observations_[2 * cam_observation_[new_cam_pose_idx][pt_idx] + 1 ]);

            cv::Mat final_points;
            //Get the traingulated points
            cv::triangulatePoints( cam0_projection_matrix, cam1_projection_matrix, point0, point1, final_points );

            cv::Mat point4d = final_points.col(0); 

            double w = final_points.at<double>(3);
            ...
            //Obtatin the non homogeneous point coordinates and check che cheirality constraint
            cv::Mat point3d = point4d.rowRange(0,3) / w;

            cv::Mat point_in_camera_0 = cam0_rotation_matrix * point3d + cam0_translation_vector;
            cv::Mat point_in_camera_1 = cam1_rotation_matrix * point3d + cam1_translation_vector;

            if (point_in_camera_0.at<double>(2) > 0 && point_in_camera_1.at<double>(2) > 0)
            {
              n_new_pts++;
              pts_optim_iter_[pt_idx] = 1;
              double *pt = pointBlockPtr(pt_idx);
    ...
    bool flag = false;
    const double max_point_dist = 100000000;
  
    
    //For each point check if its distance from the origin of coordinates is not too large
    for(int i = 0; i < num_points_; i++)
    {
      if (pts_optim_iter_[i] <= 0) continue;
      double distance = sqrt(pow(pts[i*point_block_size_], 2) + pow(pts[i*point_block_size_ + 1], 2) + pow(pts[i*point_block_size_ + 2] , 2));
    ...
    //same but for camera position
    for(int i = 0; i < num_cam_poses_; i++)
    {
      if (cam_pose_optim_iter_[i] <= 0) continue;
      double center[3]; 
      cam2center(parameters_.data() + i * camera_block_size_, center);
      double distance = sqrt(center[0] * center[0] + center[1] * center[1] + center[2] * center[2]);
  ...
  while( keep_optimize )
  {
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      ...
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
        ...
        //Get camera parameteres, set the Cost and Loss function and add a residual block
        double* camera_data = parameters_.data() + cam_pose_index_[i_obs] * camera_block_size_;
        double* point_data = parameters_.data() + num_cam_poses_ * camera_block_size_ + point_index_[i_obs] * point_block_size_;

        ceres::CostFunction* cost_funct = ReprojectionError::Create(observations_[i_obs * 2], observations_[(i_obs * 2) + 1]);

        ceres::LossFunction* loss_function = new ceres::CauchyLoss(2.0 * max_reproj_err_);


        problem.AddResidualBlock( cost_funct, loss_function, camera_data, point_data );
